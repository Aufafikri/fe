...
const DetailsPage = ({ params }: any) => {
  ...
  return (
    <>
      <Navbar />
      <div className="p-16">
        <div className="flex gap-8 shadow-xl p-4 bg-slate-900">
          <img
            src={`${product?.image}`}
            alt=""
              ...
              <button
                onClick={() => {
                  ...
                  if(!token) {
                    ...
                    toast.error("u must login first to add your product to cart!")
                    router.push("/login")
                    return;
                  }
                ...
                className="border py-4 px-8 bg-green-500 mt-6 hover:bg-green-600 transition duration-300 flex justify-center items-center gap-2 rounded-xl"
              >
                <FaCartPlus size={24} /> Add To Cart
              </button>
              <button
                onClick={() => router.push("/")}
                className="border py-4 px-10 bg-blue-500 mt-6 hover:bg-blue-600 transition duration-300 flex justify-center items-center gap-2 rounded-xl"
              >
                Checkout
              </button>
            </div>
        ...
        <div className="mt-8 dark:border-white flex gap-4 shadow-xl  dark:bg-slate-900 p-4">
          <img
            src={`http://localhost:5000${product?.Merchant.image}`}
            alt=""
          ...
          <div>
            ...
            <button
              onClick={() => {
                const slug = slugify(product?.Merchant?.storeName, {
                  lower: true,
                });
                router.push(
                  `/merchant/details/${slug}/${product?.Merchant.id}`
                );
              }}
              className="border py-2 px-6 flex items-center gap-2 hover:bg-slate-950 mt-2"
            >
              <FaShop size={20} /> See Merchant
            </button>
          </div>
          <LineVertical size={100} weight="thin" />
          <p className="w-50 text-justify">
            {" "}
            {product?.Merchant.storeDescription}
          </p>
          <LineVertical size={100} weight="thin" />
